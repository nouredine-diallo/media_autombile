...
- ✅ `[INFRA] Séparer le pipeline (cron) du serveur web dans un process distinct` — **RÉSOLU le 16 sept. 2026, voir AUDIT-PRODUCTION-READINESS-2026-09-15.md §15**. `radar-pipeline` (process PM2 dédié, `RADAR/src/pipeline-worker.ts`) exécute maintenant exactement la solution décrite ci-dessous. Vérifié en trois temps : local (140s de sondage continu pendant un vrai cycle, 100% de succès), déploiement réel (2 bugs d'outillage trouvés et corrigés en route), et **prod réelle avec le vrai chemin utilisateur** (pipeline déclenché via le vrai bouton "Lancer maintenant", 60 requêtes au vrai domaine public toutes les 10s pendant 10 minutes continues pendant le cycle réel — 60/60 en `200`, aucun timeout). Limite honnête ci-dessous (§"Limite honnête de cette solution") toujours valable, non corrigée — hors périmètre de cette passe.
  - **Analyse d'origine, conservée pour l'historique** — **trouvé le 14 sept. 2026, prod réelle** : le site devient totalement inaccessible (aucune réponse HTTP, pas une lenteur) pendant toute la durée d'un cycle pipeline (30-50 min), parce que le calcul intensif (embeddings + traduction locale, `@xenova/transformers`/ONNX) tourne dans le **même process Node** que le serveur HTTP (`next start`) — un seul thread JS, bloqué par l'inférence, ne peut plus répondre aux requêtes. Mesuré aussi sur le chemin à la demande (génération de brief à l'ouverture d'un event) : un item au contenu dégénéré (liste de titres sans ponctuation, cas réel event 171948 "Bring a Trailer") a bloqué un seul appel de traduction 40-60s d'affilée — un timeout `Promise.race`/`setTimeout` posé autour ne s'est **jamais déclenché**, la preuve empirique que Node ne peut pas interrompre un calcul natif bloquant en cours, quel que soit le wrapper JS utilisé autour.
